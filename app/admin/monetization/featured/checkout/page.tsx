...
export default function FeaturedCheckoutPage() {
  ...
  const price = PAYSTACK_PRICES_NGN.FEATURED
  ...
  return (
    <div className="max-w-2xl mx-auto py-10 px-4">
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold mt-2">Feature a Listing</h1>
        <p className="text-gray-600 mt-1">
          Get premium homepage placement with a{' '}
          <span className="font-semibold text-blue-600">✨ Featured</span> badge for 30 days.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          >
            ...
            {posts.map((p) => (
              <option key={p.id} value={p.id} disabled={p.featured}>
                {p.title} — {p.organization}
                {p.featured ? ' (already featured)' : ''}
              </option>
            ))}
          </select>
        </div>
        ...
        <div className="flex items-start gap-3 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          ...
          />
          <label htmlFor="autoRenewal" className="text-sm text-blue-800">
            <span className="font-medium">Enable 30-day auto-renewal</span>
            <br />
            <span className="text-blue-600">
              You will be reminded 7 days before renewal. Cancel anytime.
            </span>
          </label>
        </div>
        ...
        <div className="border border-gray-200 rounded-xl p-5 bg-gray-50">
          ...
          {selectedPost && (
            <div className="text-sm text-gray-500 mb-3">
              Post:{' '}
              <span className="font-medium text-gray-700">{selectedPost.title}</span>
            </div>
          )}
          {autoRenewal && (
            <div className="text-sm text-blue-600 mb-3">+ Auto-renewal enabled</div>
          )}
